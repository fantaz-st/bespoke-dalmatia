// Turbopack (the Next.js 16 default bundler) has no `raw-loader` equivalent for
// .glsl, and adding a custom rule just to import two short files isn't worth it.
// Keeping the shaders as tagged strings means zero build config.
...
vec2 backgroundCover(vec2 coords) {
    ...
    vec2 newRes = vec2(inRes.x * outRes.y / inRes.y, outRes.y);
    vec2 offset = vec2((newRes.x - outRes.x) / 2.0, 0.0);

    float outputAspect = outRes.x / outRes.y;
    float inputAspect = inRes.x / inRes.y;
    if (outputAspect >= inputAspect) {
        newRes = vec2(outRes.x, inRes.y * outRes.x / inRes.x);
        offset = vec2(0.0, (newRes.y - outRes.y) / 2.0);
    }

    offset = offset / newRes;

    return coords * outRes / newRes + offset;
}
...
void main() {
    ...
    // Index of the current column, from the X position and the total column count.
    // Use vUv (not the covered uv) so the outer stripes never get cropped.
    float currentColumnIndex = floor(vUv.x * uColumnsCount) / uColumnsCount;

    float progress = 1. - uTransitionProgress;

    // Vertical offset per column, modulated by the transition progress.
    ...
    if (coords.y < 1.) {
        ...
        // Rotate, compensating for aspect ratio
